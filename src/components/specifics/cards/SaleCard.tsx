...
import { classNames, getWearableUri, readablePrice, shortenAddress } from "@/helpers/tools";
...
export const SaleCard = (props: SaleCardProps) => {
  ...
  for (const asset of props.sale.assets) {
    ...
    } else if ( asset.class === AssetClass.ERC1155 && asset.contract === process.env.NEXT_PUBLIC_WEARABLE_CONTRACT_ADDRESS) {
  ...
  return <div className='flex flex-row flex-wrap gap-10'>
    <div key='infos' className='flex flex-col gap-2 place-content-start'>
      <div>{`Buyer ${shortenAddress(props.sale.buyer)}`}</div>
      <div className="text-center">{`id: ${props.sale.id}`}</div>
      <div className="text-center">{`price: ${price} GHST`}</div>
      <AbortSaleButton sale={props.sale} />
    </div>
    { gotchis.map((gotchi) => (<GotchiCard key={gotchi.id} gotchi={gotchi} withBackground  withBorders/>))}
    { portals.map((portal) => (<PortalCard key={portal.id} portal={portal} withBackground withBorders/>))}  
    { wearables.map((wearable) => (<WearableCard key={wearable.id} wearable={wearable} withBackground withBorders/>))}
  </div>;
};
